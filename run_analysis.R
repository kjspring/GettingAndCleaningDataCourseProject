...
subject_train <- read.table("data/raw/train/subject_train.txt", header=F) # subject
X_train <- read.table("data/raw/train/X_train.txt", header=F) # freq var
Y_train <- read.table("data/raw/train/y_train.txt", header=F) # activity
train <- cbind(subject_train, Y_train, X_train)

# Merge all the test data into one datatable
subject_test <- read.table("data/raw/test/subject_test.txt", header=F) # subject
X_test <- read.table("data/raw/test/X_test.txt", header=F) # freq var
Y_test <- read.table("data/raw/test/y_test.txt", header=F) # activity
...
features <- read.table("data/raw/features.txt", header=F)
